...
const SevereAsthmaPathway: React.FC = () => {
  ...
  const handlePreviousStage = () => {
    if (activeStageIdgreater than 1) {
  ...
  const handleNextStage = () => {
    if (activeStageIdlower than stages.length) {
        ...
        <div className="w-full lg:w-80 flex-shrink-0">
          <div className="bg-white rounded-lg shadow-md p-4 sticky top-6">
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-slate-300 hover:bg-slate-400 text-slate-800 rounded-md disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
              ← Previous Stage
            </button>
            <button
              onClick={handleNextStage}
              disabled={activeStageId greater than or equal to stages.length || activeStageId lower than or equal to 3 || activeStageId === 5}
              className="w-full px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
              Next Stage →
            </button>
            </div>

...
export const riskFactorOptions = [
  ...
  "SABA overuse (≥3 canisters/year increases risk, ≥12/year increases mortality risk)",
...
export const biologicOptions = [
  ...
  },
  {
   ...
   criteria: "Blood eosinophils ≥150-300/μL, Exacerbations in last year",
  ...
  },
  {
   ...
   criteria: "Blood eosinophils ≥150/μL OR FeNO ≥25 ppb OR maintenance OCS",
...
export const getBiologicRecommendation = (patientData: PatientData) => {
  ...
  if (totalIgE greater than or equal to 30 && totalIgE lower than or equal to 1500 && hasAllergenSensitization && exacerbations greater than or equal to 1) {
   let score = 80;
   if (isChildhoodOnset) score += 10;
   if (data.symptoms.allergenDriven) score += 10;
   if (hasNasalPolyps) score += 5;
   if (eosinophils greater than or equal to 260) score += 5;
   if (feNo greater than or equal to 19.5) score += 5;
   ...
   recommendations.push({
    ...
    strength: score greater than or equal to 90 ? "Strongly Recommended" : "Recommended",
  ...
  if (eosinophils greater than or equal to 150 && exacerbations greater than or equal to 1) {
   let score = 75;
   if (eosinophils greater than or equal to 300) score += 15; // Strongly predictive
   if (hasNasalPolyps) score += 10;
   if (isAdultOnset) score += 8;
   if (isOnOCS) score += 10;
   if (exacerbations greater than or equal to 3) score += 8; // Strongly predictive
   if (fev1lower than 65) score += 5;
   ...
   recommendations.push({
    ...
    strength: score greater than or equal to 90 ? "Strongly Recommended" : "Recommended",
    eligibility: `✓ Eosinophils ≥150/μL (${eosinophils}/μL), ✓ ≥1 exacerbation/year`,
    trialDuration: "At least 4 months",
  ...
  if (eosinophils greater than or equal to 150 && exacerbations greater than or equal to 1) {
   ...
   if (eosinophils greater than or equal to 300) score += 15;
   if (hasNasalPolyps) score += 10;
   if (isAdultOnset) score += 8;
   if (isOnOCS) score += 10;
   if (exacerbations greater than or equal to 3) score += 8;
   if (fev1lower than 65) score += 5;
  ...
  if ((eosinophils greater than or equal to 150 && eosinophils lower than or equal to 1500) || feNo greater than or equal to 25 || isOnOCS) {
   let score = 82;
   if (eosinophils greater than or equal to 300) score += 12; // Strongly predictive
   if (feNo greater than or equal to 50) score += 12; // Strongly predictive
   ...
   recommendations.push({
    ...
    eligibility: `✓ Eosinophils ≥150/μL OR FeNO ≥25 ppb OR OCS-dependent`,
    trialDuration: "At least 4 months",
  ...
  if (assessmentResults.severeAsthma && exacerbations greater than or equal to 1) {
   let score = 70;
   if (eosinophils greater than or equal to 300) score += 12; // Strongly predictive
   if (feNo greater than or equal to 50) score += 12; // Strongly predictive
   if (eosinophilslower than 150 && feNolower than 25) score += 25; // Excellent for Non-Type 2 phenotype
   if (exacerbations greater than or equal to 3) score += 8;
   
   recommendations.push({
    drug: "Tezepelumab (Anti-TSLP)",
    score: Math.min(score, 100),
    reason: `Severe asthma${(eosinophilslower than 150 && feNolower than 25) ? ', Non-Type 2 phenotype' : ', all phenotypes'}, ${exacerbations} exacerbations/year`,
    strength: (eosinophilslower than 150 && feNolower than 25) ? "Strongly Recommended (Non-Type 2)" : "Recommended",
    eligibility: `✓ Severe asthma, ✓ ≥1 exacerbation/year`,
    trialDuration: "At least 4 months",
